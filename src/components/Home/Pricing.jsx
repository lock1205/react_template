          ...
          <div key={idx} className="p-2 w-full sm:w-1/2 lg:w-1/3">
            <div className="p-10 border border-neutral-700 rounded-lg">
              <p className="text-4xl font-customFontEn mb-8">
                {option.title}
                {option.title === 'Pro' ? (
                  <span className="bg-gradient-to-r from-indigo-200 to-indigo-800 text-transparent bg-clip-text text-xl mb-4 ml-2 tracking-tighter">
                    (Most Popular)
                  </span>
                ) : (
                  ''
                )}
                {/*  option.title === 'Pro' && <span>(Most Popular)</span> 위의 문장은 다음과 같다*/}
              </p>
              <p className="mb-8">
